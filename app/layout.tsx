...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { return <html lang="pt-BR" suppressHydrationWarning><head><meta name="color-scheme" content="only light"/><script dangerouslySetInnerHTML={{__html: `(()=>{let theme="light";let motion=matchMedia("(prefers-reduced-motion: reduce)").matches?"off":"on";try{const t=localStorage.getItem("tam-theme");if(t==="dark"||t==="light")theme=t;const m=localStorage.getItem("tam-motion");if(m==="off")motion="off";}catch{}document.documentElement.dataset.theme=theme;document.documentElement.dataset.motion=motion;})()`}}/></head><body className={bodyFont.variable+' '+displayFont.variable}>{children}</body></html>; }
